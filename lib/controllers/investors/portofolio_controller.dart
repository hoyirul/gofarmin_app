...
class PortofolioController extends GetxController {
  ...
  Future<String?> getCountGoat() async {
    ...
    try {
      var url = Uri.parse(HttpHelper().getUri('/transactions/count/goats'));
  ...
  Future<String?> getAvgGoat() async {
    final prefs = await _prefs;
    try {
      var url = Uri.parse(HttpHelper().getUri('/transactions/avg/goats'));
  ...
  Future<PortofolioModel?> showPortofolio() async {
    final prefs = await _prefs;
    try {
      var url = Uri.parse(
          HttpHelper().getUri('/portofolios/${prefs.getInt('user_role_id')}'));
      ...
      if (response.statusCode == 200) {
        var decodeJson = json.decode(response.body)['data'];
        var jsonString = json.encode(decodeJson);
        return portofolioModelFromJson(jsonString);
      }
    } catch (error) {
      Get.back();
